...
extension JOAdvertCarouselView {
    ...
    func nextAdvert() {
        ...
        if indexPath.item < self.collectionView.numberOfItemsInSection(indexPath.section) - 1 {
            ...
            self.collectionView.scrollToItemAtIndexPath(NSIndexPath(forRow: 1, inSection: indexPath.section), atScrollPosition: .CenteredHorizontally, animated: false)
        }
    }
    ...
    func indexPathForNow() -> NSIndexPath {
        
        let offsetX = self.collectionView.contentOffset.x
        var index : Int = Int(offsetX / self.collectionView.frame.width)
        let remainder = offsetX - (CGFloat(index) * self.collectionView.frame.width)
        index = remainder < self.collectionView.frame.width / 3 * 2 ? index : index + 1
        return NSIndexPath(forItem: index, inSection: 0)
    }
    // 设置页码
    func  setPageindex() {
        self.pageControl.selectIndex = self.index(indexPath: indexPathForNow())
    }
}
